...
const CartItemCard = ({ product, onQuantityChange }) => {
  // Create local state for quantity
    ...
    <div className="flex items-center justify-between border rounded-lg p-4 shadow-sm mb-4">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center border rounded-md">
          <button className="px-2" onClick={decrease}>
            <img src={minus} alt="minus" className="w-3 h-3" />
          </button>
          <span className="px-3">{quantity}</span>
          <button className="px-2" onClick={increase}>
            <img src={plus} alt="plus" className="w-3 h-3" />
          </button>
        </div>

        {/* Total Price */}
        <p className="text-sm font-semibold">Rs {(product.price * quantity).toLocaleString()}</p>
      </div>
